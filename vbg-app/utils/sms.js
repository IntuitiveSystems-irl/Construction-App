// SMS utility - supports Twilio and email-to-SMS via Resend
...
// Send email-to-SMS using Resend
const sendEmailToSMS = async ({ to, text, from }) => {
  if (!resend) {
    console.warn('[SMS] Resend API key not configured, cannot send email-to-SMS');
    return { success: false, message: 'Email service not configured' };
  }

  try {
    const response = await resend.emails.send({
      from: from || process.env.RESEND_FROM_EMAIL || 'VBG <onboarding@resend.dev>',
      to,
      subject: '', // SMS gateways ignore subject
      text
    });

    console.log('[SMS] Email-to-SMS sent successfully via Resend:', response);
    return response;
  } catch (error) {
    console.error('[SMS Error] Failed to send email-to-SMS:', error);
    throw error;
  }
};

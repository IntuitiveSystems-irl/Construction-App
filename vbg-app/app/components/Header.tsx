...
export default function Header() {
  ...
  if (loading) {
    return (
      <header className="fixed bg-white shadow-md w-full z-50">
        <div className="h-1 bg-gradient-to-r from-primary-400 to-primary-700"></div>
  ...
  const isActive = (path: string) =>
    pathname === path
      ? 'relative text-teal-600 font-semibold after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-gradient-to-r after:from-teal-500 after:to-cyan-600 after:animate-underline'
  ...
  return (
    <header className={`${pathname === '/' ? 'absolute' : 'fixed bg-white shadow-md'} w-full z-50`}>
      {/* Gradient accent bar */}
          ...
          <div className="hidden md:flex items-center space-x-6">
            {isLoggedIn ? (
              <button
                ...
                className="px-6 py-3 font-medium rounded-lg bg-gradient-to-r from-teal-500 to-cyan-600 text-white hover:from-teal-600 hover:to-cyan-700 shadow-md hover:shadow-lg transition-all duration-200"
              >
                Logout
              </button>
            ) : (
              <>
                <Link
                  href="/login"
                  className="text-gray-700 hover:text-primary-700 px-5 py-3 text-sm font-medium transition-all duration-200 border border-gray-300 rounded-lg hover:border-primary-500"
                >
                  Sign in
                </Link>
                <Link
          ...
          {isLoggedIn ? (
            ...
            <div className="px-4 py-3 space-y-3 border-t border-gray-100">
              ...
              <Link
                href="/login"
                onClick={closeMobileMenu}
                className="block w-full px-4 py-2 text-center text-base font-medium text-gray-700 border border-gray-300 rounded-lg hover:border-primary-500"
              >
                Sign in
              </Link>
              <Link
                href="/register"
                onClick={closeMobileMenu}
                className="block w-full px-4 py-2.5 text-center text-base font-medium text-white bg-primary-500 rounded-lg hover:bg-primary-600 transition-all duration-200 shadow-md hover:shadow-lg"
              >
                Get Started
              </Link>
            </div>

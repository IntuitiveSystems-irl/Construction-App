...
export default function EstimatesPage() {
  ...
  const handleApproveEstimate = async (estimateId: string) => {
    try {
      const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:4001';
      const response = await fetch(`${BACKEND_URL}/api/estimates/${estimateId}/approve`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json'
  ...
  const handleRejectEstimate = async (estimateId: string) => {
    try {
      const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:4001';
      const response = await fetch(`${BACKEND_URL}/api/estimates/${estimateId}/reject`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="bg-white text-cyan-600 px-6 py-3 rounded-lg font-medium hover:bg-cyan-50 transition-colors flex items-center space-x-2"
            >
                  ...
                  {estimates.map((estimate) => (
                    <tr key={estimate.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900 font-medium">{estimate.project_name}</div>

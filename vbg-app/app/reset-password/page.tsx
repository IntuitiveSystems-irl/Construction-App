  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        ...
        const response = await fetch(`${apiUrl}/api/verify-reset-token?token=${token}`);
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              ...
              className={`group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 ${isSubmitting || message ? 'opacity-75 cursor-not-allowed' : ''}`}
            >

...
interface ContractTemplate {
  ...
  file_size?: number;
  content: string;
  template_content?: string; // Legacy field name
  created_by: number;
...
export default function ContractTemplates() {
  ...
  const handleEditTemplate = (template: ContractTemplate) => {
    ...
    setTemplateContent(template.content || template.template_content || '');
  ...
  const handleUpdateTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/admin/contract-templates/${editingTemplate.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-gradient-to-r from-teal-500 to-cyan-600 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <Link
                href="/dashboard?tab=admin"
                className="flex items-center text-white hover:text-cyan-100 mr-4"
              >
            ...
            <button
              ...
              className="flex items-center px-6 py-3 bg-white text-cyan-600 rounded-lg font-semibold hover:bg-cyan-50 transition-colors shadow-lg"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link
            ...
            className="flex items-center p-4 bg-white rounded-lg shadow hover:shadow-md transition-shadow border-l-4 border-cyan-500"
          >
            <div className="p-3 bg-cyan-100 rounded-lg mr-4">
              <FileText className="h-6 w-6 text-cyan-600" />
            </div>
            <div>
              <h3 className="font-semibold text-gray-900">Generate Contract</h3>
              <p className="text-sm text-gray-600">Create contract for existing user</p>
            </div>
          </Link>
          
          <Link
            href="/admin/send-contract-guest"
            className="flex items-center p-4 bg-white rounded-lg shadow hover:shadow-md transition-shadow border-l-4 border-teal-500"
          >
            <div className="p-3 bg-teal-100 rounded-lg mr-4">
              <Send className="h-6 w-6 text-teal-600" />
            </div>
            <div>
              <h3 className="font-semibold text-gray-900">Send Contract (Guest)</h3>
              <p className="text-sm text-gray-600">Send to anyone without account</p>
            </div>
          </Link>
          
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center p-4 bg-white rounded-lg shadow hover:shadow-md transition-shadow border-l-4 border-purple-500"
          >
            ...
            <div className="text-left">
              ...
              <p className="text-sm text-gray-600">Add new contract template</p>
            </div>
          </button>
        </div>
        ...
        {filteredTemplates.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            {!searchTerm && selectedCategory === 'all' && (
              <button
                onClick={() => setShowCreateModal(true)}
                className="flex items-center mx-auto px-4 py-2 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 transition-colors"
              >
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h3 className="text-lg font-medium text-gray-700 mb-1">{template.name}</h3>
                      <span className="inline-block px-2 py-1 bg-cyan-100 text-cyan-800 text-xs font-medium rounded">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex space-x-2">
                      <button
                        ...
                        className="flex items-center px-3 py-1 text-sm bg-cyan-100 text-cyan-700 rounded hover:bg-cyan-200 transition-colors"
                      >
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-700">Create Contract Template</h2>
                <button
              ...
              <form onSubmit={handleCreateTemplate} className="space-y-6">
                ...
                <div className="bg-cyan-50 border border-cyan-200 rounded-md p-4">
                  <h4 className="text-sm font-medium text-cyan-900 mb-2">Available Placeholders:</h4>
                  <div className="text-xs text-cyan-800 grid grid-cols-3 gap-1">
      ...
      {showEditModal && editingTemplate && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-700">Edit Contract Template</h2>
                <button
                  onClick={() => {
                    setShowEditModal(false);
                    setEditingTemplate(null);
              ...
              <form onSubmit={handleUpdateTemplate} className="space-y-6">
                ...
                <div className="bg-green-50 border border-green-200 rounded-md p-4">
                  <h4 className="text-sm font-medium text-green-900 mb-2">Available Placeholders:</h4>
                  <div className="text-xs text-green-800 grid grid-cols-3 gap-1">
                    <span>[CONTRACT_NUMBER]</span>
                    <span>[EFFECTIVE_DATE]</span>
                    <span>[CURRENT_DATE]</span>
                    <span>[OWNER_NAME]</span>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setTemplateContent(e.target.value)}
                    rows={12}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent font-mono text-sm"

...
const generateFullContractTemplate = (contract: Contract): string => {
  return `VERITAS BUILDING GROUP CONTRACT

Contract #: ${contract.id || contract.contract_id || '001'}
This Construction Contract is made as of ${new Date(contract.created_at).toLocaleDateString()} ("Effective Date") by and between ${contract.user_name || 'CLIENT'} ("Owner") of ${contract.user_email || 'CLIENT ADDRESS'} and Veritas Building Group ("Contractor") of Spokane, Washington.
...
7. Payment. Payment shall be made to Veritas Building Group. The Owner agrees to pay the Contractor as follows:
...
27. Additional Provisions. All third tier subcontractors must be approved by Veritas Building Group. All subcontractors must be verified with the department of Labor and Industries. If the subcontractor has employees they must have an active workers comp account. Failure to comply will result in the termination of the contract.
...
28. Signatories. This Contract shall be signed by the Owner ${contract.user_name || 'CLIENT'} and on behalf of Veritas Building Group by Niko Selby and shall be effective as of the date first written above.
...
export const generateContractPDF = (contract: Contract): jsPDF => {
  ...
  try {
    ...
    // For now, we'll use a red circle placeholder
    pdf.setFillColor(220, 20, 20); // Red color
    pdf.circle(pageWidth / 2, yPosition, 15, 'F');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(12);
    pdf.setFont('helvetica', 'bold');
    pdf.text('R', pageWidth / 2 - 4, yPosition + 4);
  } catch (e) {
  ...
  pdf.setFontSize(18);
  pdf.setFont('helvetica', 'bold');
  pdf.text('Veritas Building Group Contract', pageWidth / 2, yPosition, { align: 'center' });
  ...
  if (contract.signature_data && contract.signature_status === 'signed') {
    try {
      ...
      yPosition += 15;
      pdf.text('Name: Niko Selby, Veritas Building Group', margin, yPosition);
  ...
  pdf.text('Veritas Building Group | Spokane, WA | niko@veribuilds.com', pageWidth / 2, pageHeight - 15, { align: 'center' });

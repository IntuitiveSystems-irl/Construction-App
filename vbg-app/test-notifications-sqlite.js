...
/**
 * Notification Testing Script for VBG Production Server (SQLite version)
 * 
...
const notificationTypes = [
  ...
  { key: 'contract_notifications', name: 'Contract Notifications', description: 'New contracts and contract updates' },
  { key: 'estimate_notifications', name: 'Estimate Notifications', description: 'New estimates and estimate updates' },
  { key: 'invoice_notifications', name: 'Invoice Notifications', description: 'New invoices and payment updates' },
  { key: 'document_notifications', name: 'Document Notifications', description: 'Document uploads and update requests' }
];
    ...
    const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, async (err) => {
      ...
      try {
        ...
        if (tables.length > 0) {
          ...
          // Get table structure
          const columns = await dbAll(db, "PRAGMA table_info(notification_preferences)");
          ...
          await dbRun(db, `
            CREATE TABLE IF NOT EXISTS notification_preferences (
              ...
              job_assignments INTEGER DEFAULT 1,
              job_updates INTEGER DEFAULT 1,
              safety_alerts INTEGER DEFAULT 1,
              schedule_changes INTEGER DEFAULT 1,
              general_messages INTEGER DEFAULT 1,
              admin_announcements INTEGER DEFAULT 1,
              contract_notifications INTEGER DEFAULT 1,
              estimate_notifications INTEGER DEFAULT 1,
              invoice_notifications INTEGER DEFAULT 1,
  ...
  return new Promise((resolve, reject) => {
    const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, async (err) => {
      ...
      try {
        ...
        const users = await dbAll(db, 'SELECT id, name, email FROM users LIMIT 1');
        ...
        const preferences = await dbAll(db,
          `SELECT * FROM notification_preferences WHERE user_id = ?`,
          [testUser.id]
        );
...
async function testEmailNotificationSystem() {
  ...
  return new Promise((resolve, reject) => {
    const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, async (err) => {
      ...
      try {
        // Use specified test email
        const adminEmail = 'lbbusiness2025@gmail.com';
              ...
              html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                  <div style="background: linear-gradient(135deg, #14b8a6 0%, #06b6d4 100%); padding: 30px; text-align: center; border-radius: 8px 8px 0 0;">
                    ...
                    <p style="color: #ccfbf1; margin: 10px 0 0 0;">Notification System Test</p>
                  </div>
                  ...
                  <div style="padding: 30px; background: #f9fafb;">
                    ...
                    <div style="background: #ccfbf1; border-left: 4px solid #14b8a6; padding: 15px; margin: 20px 0; border-radius: 4px;">
                  ...
                  <div style="background: #0f766e; padding: 20px; text-align: center; border-radius: 0 0 8px 8px;">
                    <p style="color: #ccfbf1; margin: 0; font-size: 12px;">
  ...
  return new Promise((resolve, reject) => {
    const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, async (err) => {
      ...
      try {
        ...
        const usersWithPrefs = await dbGet(db, 'SELECT COUNT(*) as count FROM notification_preferences');
        const adminUsers = await dbGet(db, 'SELECT COUNT(*) as count FROM users WHERE is_admin = 1');
        
        log('System Statistics:', 'cyan');
        console.log(`  Total Users: ${totalUsers.count}`);
        console.log(`  Users with Preferences: ${usersWithPrefs.count}`);
        console.log(`  Admin Users: ${adminUsers.count}`);
        console.log(`  Users without Preferences: ${totalUsers.count - usersWithPrefs.count}`);
        
        // Get notification type statistics
        log('\nNotification Type Statistics:', 'cyan');
        
        for (const type of notificationTypes) {
          const enabled = await dbGet(db,
            `SELECT COUNT(*) as count FROM notification_preferences WHERE ${type.key} = 1`
          );
          const disabled = await dbGet(db,
            `SELECT COUNT(*) as count FROM notification_preferences WHERE ${type.key} = 0`
          );
          
          const total = enabled.count + disabled.count;
          const percentage = total > 0 ? ((enabled.count / total) * 100).toFixed(1) : 0;
          
          console.log(`  ${type.name}:`);
          console.log(`    Enabled: ${enabled.count} (${percentage}%)`);

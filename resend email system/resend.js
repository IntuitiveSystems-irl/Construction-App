...
export async function sendAppointmentNotification(appointmentData) {
  try {
    ...
    const response = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'VBG App <onboarding@resend.dev>',
      to: process.env.BUSINESS_EMAIL || 'niko@veribuilds.com',
...
export async function sendCustomerConfirmation(customerData) {
  try {
    ...
    const response = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'VBG App <onboarding@resend.dev>',
      to: email,
      subject: 'Thanks for your appointment request!',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
